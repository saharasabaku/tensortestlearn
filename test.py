...
dataset_dir = os.path.join(BASE_DIR,"data/dataset.npy")
# 前処理済みデータ
model_dir   = os.path.join(BASE_DIR,"data/cnn_h5")      # 学習済みモデル
# リサイズ設定
...
for class_num, label in enumerate(labels):
    ...
    # 写真のディレクトリ
    photos_dir = os.path.join(datadir,label)
    print(photos_dir)
...
#np.save(X_traindir,X_train)
#np.save(X_testdir,X_test)
#
#np.save(y_train,y_traindir)
#np.save(y_testdir,y_test)